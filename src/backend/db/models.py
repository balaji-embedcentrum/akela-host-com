...
def _refcode() -> str:
    # Short, URL-safe, upper — collision-safe enough for MVP referral codes.
...
class User(Base):
    ...
    # Phase 2 — referrals (D18)
    referral_code: Mapped[str] = mapped_column(String(16), unique=True, default=_refcode)
    referred_by_user_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
...
class Subscription(Base):
    ...
    first_period_cents: Mapped[int] = mapped_column(Integer, default=0)  # prorated (D15)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    agent: Mapped[Agent] = relationship(back_populates="subscription")


class AgentHealthSample(Base):
    """Phase 2 — uptime sampling (D17). One row per slot health probe."""

    __tablename__ = "agent_health_samples"
